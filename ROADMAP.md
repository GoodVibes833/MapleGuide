...
- 주정부 추천 1-3순위
- 연방 / EE 별도 카드
- 연방 액션 요약에서 `현재 점수 + 제안한 액션 총합 → 예상 점수` 표시
- 한국 직무 / 캐나다 직무 / 주력 경력 축 분리
- 실제 job title 입력과 직군 추론
- common ambiguous personas now have explicit planner directions:
  - `server -> food service supervisor or cook`
  - `retail sales -> retail supervisor or office coordinator`
  - `warehouse -> inventory coordinator or dispatcher`
...
- province card 안의:
  ...
  - concrete plan A/B/C
  - timeline
- jurisdiction page 상단 personalized carry-over block:
  - 메인 추천에서 보던 사람 기준 요약
  - why/strengths/actions
  - EE bridge
  - stream 현실 가이드 일부
  - concrete plans / short timeline
- 브라우저 세션 기준 입력 저장 / 불러오기 / 초기화
- GitHub Pages 배포
- GA4 준비
- 프로젝트 컨텍스트 문서화
...
현재는 직군을 많이 늘렸고 common persona planner focus도 추가했지만, 아직도 완전한 `title -> NOC -> 주별 rule` 수준은 아닙니다.
...
- `src/config/province-stream-rules.js`로 1차 scaffold는 이미 들어갔습니다.
- Ontario / Alberta / Nova Scotia / PEI / Saskatchewan / Newfoundland and Labrador 규칙 카드가 recommendation detail 안에 출력됩니다.
- 아직 `title -> NOC-like candidate -> exact stream hit` 수준은 아니고, 다음 단계는 규칙 엔진 정밀화입니다.
...
- 메인 추천에서 `이 지역 먼저 보기`로 들어가도 같은 사람 기준 흐름이 이어진다고 느껴진다
...
- planner language는 상당 부분 들어갔습니다.
- 다음 단계는 각 플랜을 province rules와 더 촘촘히 묶고, 직무 title 후보와 직접 연결하는 것입니다.
